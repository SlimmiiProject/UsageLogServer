...
export const AllDevices = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      await requestAllDevices(controller)
      await requestAllUsers(controller)
      setisloading(false);
    }
  ...
  const requestAllDevices = async (controller: AbortController) => {
  ...
  const requestAllUsers = async (controller: AbortController) => {
    AdminUtil.getUsers(controller, 0, 15).then((result: userResponseData) => {
      ...
      if (pages > 1) {
        for (let x = 1; x < pages; x++) {
          AdminUtil.getUsers(controller, x, 15).then((result: userResponseData) => {
            setUsers(users => {
              return [...users, ...result.data];
            });
          });
        ...
        <Dialog open={open} onClose={handleClickClosed} maxWidth="lg">
          ...
          <DialogContent>
            <FormControl>
              ...
              >
                {users.map((u) => (<MenuItem key={`${u.userId}`} value={`${u.userId}`}>{u.userId} - {u.firstname} {u.lastname}</MenuItem>))}
              </Select>
            </FormControl>
          </DialogContent>
          <DialogActions>
